...
const MaintainFastFood = () => {
  const { fastFood, refetchItems } = useItemsContext();
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/add-fast-food`,
        itemDetails
      );
  ...
  const handleDelete = async (item) => {
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/delete-fast-food/${item._id}`
      );
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
                    <div>
                      ...
                      <input
                        ...
                        type="text"
                        placeholder="Italian pizza 12 inch"
                        {...register("name", {
...
export default MaintainFastFood;
