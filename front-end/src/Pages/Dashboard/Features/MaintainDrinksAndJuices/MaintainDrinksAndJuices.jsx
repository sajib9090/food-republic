...
const MaintainDrinksAndJuices = () => {
  const { drinksAndJuices, refetchItems } = useItemsContext();
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/add-drinks-juices`,
        itemDetails
      );
  ...
  const handleDelete = async (item) => {
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/delete-drinks-juices/${item._id}`
      );
...
export default MaintainDrinksAndJuices;
